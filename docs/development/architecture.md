...
| `geoserver_manager/gui/tab_workspaces.py`, `tab_datastores.py`, `tab_coveragestores.py`, `tab_cascaded.py`, `tab_layers.py`, `tab_layergroups.py`, `tab_styles.py`, `tab_gwc.py`, `tab_server.py` | One mixin per resource type (the Server tab: one row per server-wide setting, no Add or Delete): load / add / edit / delete (layers also: publish, add to QGIS, preview; layer groups also: add to QGIS; coverage stores also: publish a coverage; cascaded stores also: publish / view / delete a remote layer; tile cache: configure, truncate, stop caching) |
...
- **Server calls go through the helpers on the dialog**, never hand-rolled in a mixin:
  ...
  | `_delete_many(kind, [(label, fn)], reload_fn, counted, cascade=…, verb=…, done=…)` | confirm + run in a task of its own slot (`_delete`), with progress, and report one or many deletions. A tab switch or F5 never stops it, Cancel does, one batch runs at a time, and it reloads only the tab it started from; `counted` is the tab's `n -> translate(ctx, "%n layer(s)", None, n)`, so each locale gets its plural forms ([translations](translation.md)); `verb`/`done` for a tab whose action is not a delete ("stop caching" / "removed from the cache") |
